...
3) JSP 페이지에서 사용하기
    - taglib 지시문을 사용하여 라이브러리 정보를 가져온다.
      &li;@ taglib uri="JSTL 라이브러리명" prefix="접두사" %>
    - JSTL 라이브러리 접두사를 이용하여 태그를 사용한다.
    - &li;접두사:태그명 속성명="값" 속성명="값"/>
    
JSTL 라이브러리들
1) Core : 변수, 조건문, 반복문 등을 다루는 태그
    &lt;%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%> <%-- prefix는 보통 c --%>
2) XML : XMl을 다루는 태그
    &lt;%@ taglib uri="http://java.sun.com/jsp/jstl/xml" prefix="x"%> <%-- prefix는 보통 x --%>
...
<%-- 접두어는 마음대로 바꿔도 되지만 uri는 바꿔선 안된다. 그러나 접두어도 개성살리지 말고 사용해라 --%>
</pre>
