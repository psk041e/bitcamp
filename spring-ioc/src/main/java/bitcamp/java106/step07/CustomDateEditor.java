...
//    너무 번거롭다.
// => 그래서 자바는 PropertyEditor를 미리 구현한 PropertyEditorSupports 라는 클래스를 제공한다.
...
public class CustomDateEditor extends PropertyEditorSupport {
    ...
    public void setAsText(String text) throws IllegalArgumentException {
        System.out.println("CustomDateEditor.setAsText()"); // 순서확인
        // 파라미터로 넘어온 String 타입의 프로퍼티 값을
        // 원하는 타입의 값(java.sql.Date)으로 바꿔 내부에 저장한다.
        // => 그러면 스프링 IoC 컨테이너는 이 값을 꺼내서 객체에 주입할 것이다.
        this.setValue(Date.valueOf(text));
    }
    
    // 언제 어떤 메서드가 호출 되는지 확인하기 위해 상속받은 메서드를 오버라이딩(재정의)해 보자!
    @Override
