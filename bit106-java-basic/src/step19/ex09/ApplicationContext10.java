// @Component 애노테이션이 붙은 클래스만 객체를 생성한다.(제약을 가함)
package step19.ex09;
...
public class ApplicationContext10 { 
    ...
    public void findAndInstantiateClasses(File dir, String packageName) throws Exception { 
        ...
        for (File f : files) {
            ...
            if (obj != null) {
                this.objPool.put(getComponentName(clazz), obj); // 객체를 저장하는데 해당 클래스 이름으로 저장하겠다.
            }
        } 
    }     
    
    private String getComponentName(Class clazz) throws Exception { // 지정한 이름으로만 찾아야 한다.
        Component anno = (Component) clazz.getAnnotation(Component.class); // 컴포넌트라는 애노테이션이 든 클래스를 가져온다.
    ...
    private Object createObject(Class clazz) throws Exception {
        ...
        if (!isComponent(clazz)) // 컴포넌트가 아니면 생성하지 말아라
            return null;
    ...
    private boolean isComponent(Class clazz) throws Exception {
        ...
        // 컴포넌트가 붙지 않은 클래스는 자동으로 생성할 대상이 아니다.
