...
public class Score implements Serializable {
    ...
    // => sum과 aver 는 위의 변수를 계산해서 그 결과를 저장하는 변수이다.
    // sum과 aver는 실제 출력해서는 안된다.
    // => 조작이 될 수 있다. 그래서 계산 결과를 담는 인스턴스 변수는 출력하는게 아니다!
    // 이렇게 다른 인스턴스 변수의 값을 가지고 계산한 결과를 저장하는 변수인 경우
    // sirialize 대상에서 제외해야 한다.
    // 왜? 값의 왜곡을 막기 위함이다.
    // 그럼 sum과 aver는 어떻게 처리해야 하는가?
    // => serialize 데이터를 읽는 쪽에서 데이터를 다 읽은 후 계산을 수행해서값을 처리해야 한다. 
