...
/* table drop 은 roolback이 되지 않는다. insert/delete/update에 대해서만 가능하다.*/
...
select no, name
from test1
where working <> 'Y'; /* 되지 않는 dbms도 있다.*/
...
``` /* 오라클은 테이블명을 from dual이라고 명시해주어야만 한다.*/
...
```
select 5 between 3 and 5; /* 3이상 5이하의 값*/
```
...
select *
from test1
where name like 's0_'; /* 오라클에서는 _가 아니라 ?를 사용한다.*/
```
