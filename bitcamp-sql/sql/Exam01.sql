...
- 메모리 크기가 가변적이라서 데이터 위치를 찾을 때 시간이 오래 걸린다.
  ...
  but 요새는 다루기가 편하기 때문에 속도가 떨어져도 varchar를 많이 쓴다.
...
> create table test1(
  ...
  /* test_pk는 제약조건 이름, 이름은 생략해도 된다. 자신이 원하는 이름을 지정하면 된다.
 보통 각각의 제약조건을 구분하기 위해서 라벨을 붙여주는 것이다.*/
...
> create table test1(
  ...
  ); /* unique의 제약조건 이름은 보통 uk(unique key)를 사용한다.*/
...
- 단점, 이런 이유로 이름으로 검색할 때 찾기 속도는 빠르지만,
  입력,변경,삭제 속도는 느리게 된다. 
  -> 따라서 자주 검색하는 컬럼에 대해서만 인덱스를 만드는것이 좋다.
  -> 게시판을 만들 때 제목으로만 찾기 때문에 보통 제목에 인덱스를 건다.
