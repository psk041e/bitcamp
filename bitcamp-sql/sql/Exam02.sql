...
``` /* insert할 때 다른 테이블의 결과값을 가져와서 집어넣을 수 있다. 
     * 다만, 집어넣으려는 컬럼의 데이터타입과 개수가 일치해야 한다.*/
...
연습2:
```
insert into test1(name, tel) values('xxx','11111');
insert into test1(name, tel) values('yyy','22222');
insert into test1(name, tel) values('zzz','22222');
...
delete from test1 where no=2 or no=6;
